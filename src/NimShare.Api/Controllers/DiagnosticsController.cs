...
public class DiagnosticsController : Controller
{
    ...
    /// <summary>Global error page — surfaces the exception message so operators
    /// aren't blind when the site 500s. Only the message + status are shown
    /// (no stack trace) to avoid leaking internals to end users.</summary>
    [Route("/error")]
    [AllowAnonymous]
    public IActionResult Error()
    {
        ...
        var type = feature?.Error?.GetType().FullName ?? "";
        ...
        return Content(
            ...
            "<h1>Server error</h1>" +
            "<p>Something went wrong on the server. Details below help the admin fix it.</p>" +
            $"<pre>{System.Net.WebUtility.HtmlEncode(type)}\n\n{System.Net.WebUtility.HtmlEncode(msg)}</pre>" +
